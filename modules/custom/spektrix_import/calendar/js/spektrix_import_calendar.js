...
$.fn.zabuto_calendar_language = function (lang) {
  ...
  Drupal.behaviors.sydCalendar = {
    attach: function (context, settings) {
      ...
      // Build calendar
      $("#sydCalendar").zabuto_calendar({        
        data: eventData,
        action: function () {
          return showEvents(this.id);
        }
      });
        
      // Fetches performances on given date
      function showEvents(id) {

        var date = $("#" + id).data("date");
        
        var hasEvent = $("#" + id).data("hasEvent");
        if (!hasEvent) { // No event, terminate here
          return false;
        }
        ...
        window.location.href = '/whats-on/date?date=' + date.replace('-', '');
        
//        // Fetch performances and display them below calendar
//        $.ajax({
//          url: '/calendar_perfs/' + date,
//          async: false,
//          success: function(data){
//            var performances = $('#sydCalendarPerformances'),
//                overlay = $('#js-olContainer');
//
//            //console.log(data);
//            performances.find('.views-element-container').remove();
//
//            performances.append(data).hide().fadeIn('fast', function(){
//                overlay.animate({
//                    scrollTop: performances.offset().top
//                }, 200);
//            });
//          }
//        });        
